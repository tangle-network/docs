...
| ---------------------------------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Published Sandbox SDK              | `@tangle-network/sandbox` `BackendType`     | OpenCode, Claude Code, Kimi Code, Codex, AMP, Factory Droids, Pi, Hermes, Forge, ACP, Cursor, and CLI base.                                                                                     |
| Public Sandbox UI picker           | `sandbox-ui` harness picker                 | OpenCode, Claude Code, Codex, AMP, Factory Droids, Kimi Code, Hermes, and CLI base. The picker defers SDK backends such as Pi, Forge, ACP, and Cursor until copy and compatibility policy land. |
...
The public Sandbox UI picker can expose a smaller subset than the SDK. Product docs should say which surface they mean and should prefer `/api/capabilities` for live operator surfaces.
...
> The sandbox supports multiple harnesses. OpenCode is one adapter beside Claude Code, Kimi Code, Codex, AMP, Factory Droids, Pi, Hermes, Forge, ACP, Cursor, CLI base, and blueprint sidecar images that may advertise a smaller set through `/api/capabilities`.
