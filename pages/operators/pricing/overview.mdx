...
**Quote requester binding (v0.13.0).** Both `QuoteDetails` (service quotes) and
`JobQuoteDetails` (per-job quotes) now have `requester` as the FIRST field of the EIP-712
typed data. The protocol enforces `requester == msg.sender` and rejects wildcard
`requester == address(0)`. Your quote server MUST sign per-caller quotes with `requester`
populated to the customer's address, otherwise the customer cannot redeem your quote
on-chain. This also defeats mempool front-running: another `permittedCaller` (or anyone
watching the mempool) can no longer lift your signed digest and consume it.
