...
- Familiarity with running the Blueprint Manager runtime
- Knowledge of blueprint concepts and service lifecycles
...
1. **Registration**: When you register as an operator for a blueprint, you provide your pricing service address in your preferences
   - **Note**: If your RPC server address changes, update it on-chain via `updateOperatorPreferences` on the `Tangle` contract (see `ITangleOperators`)
...
- **Asset Types**: Native (address(0)) or ERC20 (token address)
