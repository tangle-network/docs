...
**When it's skipped:** Only in two cases. When a compliance flag is set ([ZDR](/gateway/zdr), [no-train](/gateway/no-train), or data residency), because operators can't verify compliance. And when `providerOptions.gateway.order` is supplied.
