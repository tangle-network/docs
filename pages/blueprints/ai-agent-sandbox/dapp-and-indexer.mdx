...
Do not name the product after one harness. The public Sandbox SDK supports OpenCode plus 11 peer backend types: Claude Code, Kimi Code, Codex, AMP, Factory Droids, Pi, Hermes, Forge, ACP, Cursor, and CLI base. The public Sandbox UI picker exposes a smaller product-policy subset. The AI Agent Sandbox blueprint's current all-harness sidecar advertises Claude Code, Codex, OpenCode, Kimi Code, and Gemini CLI. The product boundary is the sandbox service instance and operator API.
