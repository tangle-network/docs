...
For a field-by-field reference, see [Profile Schema](/workbench/profile-schema). The schema is harness-aware; each backend maps the shared profile fields to its own config surface. See [Sandbox Harnesses](/infrastructure/harnesses) for the centralized sandbox list and the smaller AI Agent Sandbox blueprint sidecar list.
