...
Workbench profiles configure the selected agent harness, model routing, per-agent prompts, and tool permissions. OpenCode sits beside 11 other public Sandbox SDK backend types: Claude Code, Kimi Code, Codex, AMP, Factory Droids, Pi, Hermes, Forge, ACP, Cursor, and CLI base. Protocol-backed service instances must still read their own `/api/capabilities` response.
