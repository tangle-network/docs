...
| `disputeSlash(slashId, reason)` `payable`         | Slashed operator (must post bond), or `SLASH_ADMIN_ROLE` (admin cannot self-dispute their own proposal) | [Slashing.sol](https://github.com/tangle-network/tnt-core/blob/main/src/core/Slashing.sol) |
...
receiver.activateMessenger();
```

**Receiver enforcement (v0.13.0).** `receiveMessage` now reverts if the L2 slasher returns
