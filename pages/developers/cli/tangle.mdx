...
Blueprint definition manifests can include `metadata.execution_profile.confidentiality` to declare `any`,
`tee_preferred`, `tee_required`, or `standard_required`. See
...
`--protocol tangle-evm` is accepted as an alias for `--protocol tangle`.
