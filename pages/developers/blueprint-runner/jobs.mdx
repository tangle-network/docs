...
- The job takes a single ABI-encoded input parameter `x` extracted by `TangleEvmArg<u64>`
- The job returns a `TangleEvmResult<u64>`, which the runner ABI-encodes for submission
