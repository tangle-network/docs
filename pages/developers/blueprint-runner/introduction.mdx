...
Blueprint Runners are the operator-side runtime that executes blueprint jobs. They coordinate between producers (event sources), routers (job dispatch), consumers (result sinks), and background services.
...
[Producers](/developers/blueprint-runner/producers) listen for events (such as on-chain logs) and prepare them for processing. They convert raw event data into a format that can be processed by your job handlers.
...
The Blueprint Runner follows a well-defined lifecycle:
