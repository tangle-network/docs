...
First, you define the producer you want to use. For example, a `TangleEvmProducer` that polls `JobSubmitted` events for a specific `serviceId` and converts them into `JobCall`s.
...
let tangle_producer = TangleEvmProducer::new(tangle_client, service_id);
...
#### Tangle Producer (`tangle-evm`)

<GithubFileReaderDisplay
  url="https://github.com/tangle-network/blueprint/blob/v2/crates/tangle-evm-extra/src/producer.rs"
  fromLine={1}
  toLine={120}
  title="TangleEvmProducer: job-call producer from EVM logs"
/>
