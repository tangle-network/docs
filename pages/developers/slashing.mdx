...
```
   ...
   [ Disputed ]   cancelSlash   [ Executed ]
     |    \           |
     |     \          v
     |    cancelSlash (bond claimable)   [ Cancelled ]
     v
...
### `proposeSlash(serviceId, operator, slashBps, evidence)` (`nonReentrant`)
...
### `disputeSlash(slashId, reason)` `payable` (`nonReentrant`)

The operator, a `SLASH_ADMIN_ROLE` holder, or a BSM-declared dispute origin contests the slash within the dispute window.

The operator must post `config.disputeBond` in native asset. `SLASH_ADMIN` posts no bond. A BSM-declared bondless dispute origin also posts no bond, unless the blueprint itself authored the slash, in which case the bondless path closes and the full `disputeBond` is required. The dispute snapshots `config.disputeResolutionDeadline` onto the proposal (`disputeDeadline = block.timestamp + deadline`), so a later admin-driven shrink of the live config cannot retroactively shorten the operator's review window.
...
- A `Disputed` proposal whose `disputeDeadline + TIMESTAMP_BUFFER` has elapsed (symmetric 15s
  buffer with the dispute window, so a sequencer with timestamp influence cannot sandwich the
  deadline tick to land an `executeSlash` before the operator's dispute window closes)
...
| `disputeSlash`      | The slashed operator (must post bond), `SLASH_ADMIN_ROLE` (no bond), or a BSM-declared dispute origin (no bond, unless the blueprint authored the slash)                                                                                                          |
...
| `claimDisputeBond`  | Anyone with a credited balance in `_pendingDisputeBondRefunds` (the disputer after a cancel, or the treasury after a failed push). Withdraws that balance.                                                                                                        |
...
4. **After disputing**, escalate to `SLASH_ADMIN`. Provide your evidence off-chain. The admin has up to `disputeResolutionDeadline` to call `cancelSlash` (credits your bond back to you, claimable) or do nothing (slash auto-executes, bond forfeit).
5. **Claim your bond.** `cancelSlash` does NOT push the bond back to you. It credits `_pendingDisputeBondRefunds[disputer]`. You must call `claimDisputeBond()` yourself to receive the ETH. A cancelled slash with an unclaimed bond is the common operator mistake here.
6. **Watch your `_operatorPendingSlashCount`.** If it stays elevated long after a dispute resolved, raise a ticket with protocol governance to investigate.
...
3. **If the dispute is valid**: `cancelSlash(slashId, "summary")`. The bond is credited to the disputer's pull-claimable balance (`_pendingDisputeBondRefunds`) and pending counters decrement. Tell the disputer to call `claimDisputeBond()`; the protocol never pushes the ETH back for them.
...
- A legitimate dispute is credited back to the operator on `cancelSlash`. The refund is pull-based: `cancelSlash` only credits `_pendingDisputeBondRefunds[disputer]`, and the operator calls `claimDisputeBond()` to withdraw it.

Edge cases (handled by `_settleDisputeBond`):

- If `_treasury == address(0)` when a forfeit would occur, the bond is credited to `_pendingDisputeBondRefunds[disputer]` instead of being stranded, and the disputer drains it with `claimDisputeBond()`. It is only restored on the proposal in the unreachable case where the disputer is also `address(0)`. Configure the treasury before any forfeit-eligible slash auto-fails.
- If the treasury push (`treasury.call`) reverts, the bond is credited to `_pendingDisputeBondRefunds[treasury]`, NOT restored on the proposal. There is no push to the disputer anywhere in `_settleDisputeBond`; every refund path is pull-based. The bond is never silently lost, but it is never delivered without the credited address calling `claimDisputeBond()`.
...
- `_executeSlashOnStaking` loads the commitment snapshot taken at `proposeSlash` time (`_slashCommitmentSnapshots[slashId]`), not live storage, so commitment changes after the proposal cannot shrink the slash.
