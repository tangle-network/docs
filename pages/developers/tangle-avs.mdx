---
title: Build a Tangle Blueprint
description: Write a job + router, run it end-to-end, and connect it to the on-chain job system.
---

import GithubFileReaderDisplay from "/components/GithubFileReaderDisplay";

# Build a Tangle Blueprint

This guide walks through the minimum shape of a Tangle blueprint:

- One or more **jobs** (async handlers)
- A **router** that maps `jobIndex → handler`
- A **runner** (operator-side process) that consumes job calls and submits results

## Prerequisites

- Rust toolchain: https://www.rust-lang.org/tools/install
- `cargo-tangle` CLI: [CLI Installation](/developers/cli/installation)

## A Minimal Job + Router (Hello Tangle)

The following example defines a single job (`CREATE_DOCUMENT_JOB`) that accepts a `DocumentRequest` and returns a `DocumentReceipt`.

<GithubFileReaderDisplay
  url="https://github.com/tangle-network/blueprint/blob/v2/examples/hello-tangle/src/lib.rs"
  fromLine={1}
  toLine={62}
  title="Hello Tangle: job handler + router"
/>

## Run End-to-End (Local Harness)

The SDK includes an Anvil-backed harness that spins up a local EVM node seeded with a known protocol state, runs the blueprint runner, submits a job, and waits for `JobResultSubmitted`.

```bash
cargo test -p hello-tangle-blueprint --test anvil -- --nocapture
```

<GithubFileReaderDisplay
  url="https://github.com/tangle-network/blueprint/blob/v2/examples/hello-tangle/tests/anvil.rs"
  fromLine={12}
  toLine={78}
  title="Hello Tangle: end-to-end test harness (submit job, wait for result)"
/>

## Run Against a Real Deployment

To run as an operator against a live RPC, you typically:

1. Configure contract coordinates and IDs (blueprint/service).
2. Start the runner via the CLI with the `tangle-evm` protocol selected.

Deployment-dependent addresses and endpoints are published per environment:

- [Endpoints and Integration](/developers/endpoints)

## Next Reading

- [Blueprint Runner](/developers/blueprint-runner/introduction)
- [Tangle EVM Client Context](/developers/blueprint-contexts/tangle-client-context)
- [System Architecture](/developers/system-architecture/overview)
